...
const AdminBillOrder = ({ tableId, acPercentage }) => {
  ...
  console.log("modifiedCurrentOrder", modifiedCurrentOrder);
  ...
  const removeFromOrder = (product) => {
    setCurrentOrder((prevOrder) => {
      ...
      if (existingItem) {
        ...
        // Update tapped products if the product quantity becomes 0
        if (existingItem.quantity === 1) {
          setTappedProducts((prevTapped) =>
            prevTapped.filter((id) => id !== product._id)
          );
        }
  ...
  const addToOrder = useCallback(
    ...
    [setCurrentOrder, setTappedProducts, setTapCounts]
  );
  ...
  useEffect(() => {
      const fetchSections = async () => {
        try {
          ...
          console.log("data", sectionsResponse.data);
  ...
  const printKOT = async (items, tableName) => {
    ...
    try {
      ...
      //     type: 'success',
      //     text1: 'Success',
      //     text2: 'KOT printed successfully!',
      //     position: 'top',
      //     visibilityTime: 2000, // Dismiss after 2 seconds
      //   });
  ...
  const showTables = () => {
    ...
    if (sectionId) {
      router.push(`/bill?sectionId=${sectionId}`); // Navigate to bill page with sectionId as a query parameter
    } else {
...
${index.message}`;
    ...
    `);
      ...
      // Print the bill
      const response = await axios.post(`http://192.168.0.109:5000/print-bill`, {
        billContent,
      });
      ...
      <View style={{ backgroundColor: "white", padding: 10 }}>
        <View style={styles.outerContainer}>
          <View style={styles.container}>
            ...
            <TouchableOpacity style={styles.home} onPress={toggleHome}>
              <FontAwesome name="home" size={32} style={styles.home} />
            </TouchableOpacity>
          </View>
                ...
                .map((orderItem) => (
                  <View key={orderItem._id} style={styles.orderItemContainer}>
                    ...
                    {/* 
                      ...
                      onChangeText={(value) =>
                        handleSelectChange(orderItem._id, value)
                      }
                      style={[
                        styles.tasteInput,
                        isFocused && styles.tasteInputFocused,
                      ]}
                    ...
                    {selectedTastes[orderItem._id] === "other" && (
                      <TextInput
                        ...
                        onChangeText={(value) =>
                          handleNewTasteChange(orderItem._id, value)
                        }
                        style={styles.newTasteInput}
                      />
                    )}
        ...
        >
          ...
          >
            ...
            >
              ...
              {/* Search Input */}
              <View style={{ paddingHorizontal: 2, marginVertical: 8 }}>
                <TextInput
                  value={searchInput}
                  ...
                  style={{
                    paddingHorizontal: 8,
                    paddingVertical: 4,
                    borderColor: "gray", // Use a distinctive color to ensure it's visible
                    borderWidth: 2,
                    borderRadius: 4,
                    fontSize: 12,
                    width: "35%",
              ...
              >
                ...
                >
                  MenuCart: {calculateTotal().totalQuantity}
                </Text>
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  outerContainer: {
    ...
    marginTop: height * 0.02, // 10% of screen height for top margin
  },
  container: {
  ...
  sectionName: {
    ...
    marginTop: height * -0.02, // Negative margin adjusted based on height
  },
  tableName: {
