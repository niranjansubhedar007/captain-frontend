...
import AdminBillOrder from "./adminBillOrder";
...
const OrderPage = () => {
      ...
      try {
        if (tableId) {
          const tableResponse = await axios.get(
            `http://192.168.0.109:5000/api/table/tables/${tableId}`
          );
          const table = tableResponse.data;

          const sectionResponse = await axios.get(
            `http://192.168.0.109:5000/api/section/${table.section._id}`
          );
  ...
  return (
    <View>
      <AdminBillOrder tableId={tableId} acPercentage={acPercentage} />
      {/* <Text>{tableId}</Text> */}
    </View>
  );
};
